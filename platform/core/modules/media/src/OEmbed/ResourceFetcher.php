  ...
  public function fetchResource($url) {
    ...
    elseif (strstr($format, 'text/javascript') || strstr($format, 'application/json') || strstr($format, 'text/html')) {
      try {
        ...
        if (!is_array($data)) {
          throw new ResourceException('The oEmbed resource was not formatted as expected.', $url);
        }
      }
      catch (InvalidDataTypeException $e) {
        throw new ResourceException('The fetched resource did not have a valid Content-Type header or could not be parsed as JSON', $url);
      }
    }
